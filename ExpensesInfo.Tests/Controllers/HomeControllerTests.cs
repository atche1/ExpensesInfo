...
using ExpensesInfo.Controllers;
...
namespace ExpensesInfo.Tests.Controllers
{
    public class HomeControllerTests
    {
        ...
        public async Task Expenses_Should_Return_View_With_Model()
        {
            ...
            mockSvc.Setup(s => s.GetAllTypesAsync()).ReturnsAsync(new
            List<ExpenseType>());
            mockSvc.Setup(s => s.GetAllAsync(null)).ReturnsAsync(new
            List<Expense> { new Expense { Value = 1 } });
            mockSvc.Setup(s => s.GetTotalAsync(null)).ReturnsAsync(1m);
            var controller = new HomeController(mockSvc.Object);
            var result = await controller.Expenses(null) as ViewResult;
            Assert.NotNull(result);
            Assert.IsType<List<Expense>>(result.Model);
            var model = (List<Expense>)result.Model!;
            Assert.Single(model);
        }
        [Fact]
        ...
        CreateEditExpenseForm_Valid_New_Should_Redirect_And_Call_Create()
        {
            ...
            var result = await controller.CreateEditExpenseForm(model) as
            RedirectToActionResult;
            Assert.NotNull(result);
            Assert.Equal(nameof(HomeController.Expenses), result.ActionName);
            mockSvc.Verify(s => s.CreateAsync(It.IsAny<Expense>()),
            Times.Once);
        }
    }
}