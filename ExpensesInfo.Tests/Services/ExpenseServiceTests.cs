...
namespace ExpensesInfo.Tests.Services
{
    public class ExpenseServiceTests
    {
        ...
        {
            using var db =
            DbFactory.CreateInMemory(nameof(GetAllAsync_Should_Filter_By_Type));
            var typeFood = new ExpenseType { Name = "Food" };
            var typeFuel = new ExpenseType { Name = "Fuel" };
            db.ExpenseTypes.AddRange(typeFood, typeFuel);
            db.Expenses.AddRange(
            new Expense
            {
                Value = 10,
                ExpenseType = typeFood,
                Date =
            DateTime.Today,
            ...
            onlyFood.Should().HaveCount(1);
            onlyFood[0].ExpenseTypeId.Should().Be(typeFood.Id);
        }
        [Fact]
        public async Task CreateAsync_Should_Add_Expense()
        {
            using var db =
            DbFactory.CreateInMemory(nameof(CreateAsync_Should_Add_Expense));
            var svc = new ExpenseService(db);
            var e = new Expense
            {
                Value = 15,
        ...
        public async Task UpdateAsync_Should_Modify_Fields()
        {
            ...
            var updated = await db.Expenses.SingleAsync();
        ...
        {
            using var db =
            DbFactory.CreateInMemory(nameof(GetTotalAsync_Should_Sum_Filtered));
            var t1 = new ExpenseType { Name = "A" };
            var t2 = new ExpenseType { Name = "B" };
            db.ExpenseTypes.AddRange(t1, t2);
            db.Expenses.AddRange(
            new Expense
            {
                Value = 10,
            ...
            await db.SaveChangesAsync();
            var svc = new ExpenseService(db);
            var sumT1 = await svc.GetTotalAsync(t1.Id);
            sumT1.Should().Be(15);
        }
    }
}